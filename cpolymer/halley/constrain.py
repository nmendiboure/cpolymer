...
from . import vectors
from .vectors import V, zero, equal, EPSILON
import math
...
class Duality(Constraint):
    ...
    def combine(self, other):
        ...
        if a and b:
            return self
        if a:
            return Unity(self.pointA)
        if b:
            return Unity(self.pointB)
        return Nowhere()
    ...
    def accepts(self, point):
        ...
        if self.pointA == point:
            return True
        if self.pointB == point:
            return True
        return False
...
class Linear(Constraint):
    ...
    def combine(self, other):
        ...
        if isinstance(other, Anywhere):
            return self
        if isinstance(other, Nowhere):
            return other
        if isinstance(other, Unity):
            return other.combine(self)
        if isinstance(other, Duality):
            return other.combine(self)
        if isinstance(other, Linear):
            return self._linear_linear(other)
        if isinstance(other, Planar):
            return other.combine(self)
        if isinstance(other, Circular):
            return other.combine(self)
        if isinstance(other, Spherical):
            return other.combine(self)
        return Nowhere()
    ...
    def _linear_linear(self, other):
        ...
        crossS = span * dirS
        if crossS.zero():
            return Unity(other.pointA)
        crossXSO = crossO * crossS
        if not crossXSO.zero():
            return Nowhere()
...
class Planar(Constraint):
    ...
    def combine(self, other):
        ...
        if isinstance(other, Duality):
            return other.combine(self)
        if isinstance(other, Linear):
            return self._planar_linear(other)
        if isinstance(other, Planar):
            return self._planar_planar(other)
        if isinstance(other, Circular):
            return other.combine(self)
        if isinstance(other, Spherical):
            return other.combine(self)
...
def closest_point_on_circle(center, normal, radius, p):
    ...
    spoke = planar - center
    if not spoke:
        spoke = V(1, 0, 0) * normal
    if not spoke:
        spoke = V(0, 1, 0) * normal
...
class Circular(Constraint):
    ...
    def combine(self, other):
        ...
        if isinstance(other, Duality):
            return other.combine(self)
        if isinstance(other, Linear):
            return self._circular_linear(other)
        if isinstance(other, Planar):
            return self._circular_planar(other)
        if isinstance(other, Circular):
            return self._circular_circular(other)
        if isinstance(other, Spherical):
            return other.combine(self)
        return Nowhere()
    ...
    def _circular_planar(self, other):
        ...
        if isinstance(intersect, Nowhere):
            return intersect
...
class Spherical(Constraint):
    ...
    def combine(self, other):
        ...
        if isinstance(other, Duality):
            return other.combine(self)
        if isinstance(other, Linear):
            return self._spherical_linear(other)
        if isinstance(other, Planar):
            return self._spherical_planar(other)
        if isinstance(other, Circular):
            return self._spherical_circular(other)
        if isinstance(other, Spherical):
            return self._spherical_spherical(other)
        return Nowhere()
    ...
    def _spherical_circular(self, other):
        ...
        if not isinstance(intersect, Circular):
            return intersect
