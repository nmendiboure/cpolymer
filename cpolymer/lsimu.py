...
class LSimu:
    ...
    def coarse(self, avoid_type=[], root="./cg", nlevel=2, periodic=True, lastcmd="lammps", **args):
        ...
        for nmol, molecule in enumerate(self.molecules):
            ...
            if molecule.angle:
                angle_bond = True
            Nbead += len(nt)
            P = Polymer(N=len(nt), type_bead=nt, gconstrain=[self.Coarse.box], liaison=molecule.liaison,
                        angle_def=molecule.angle_def, angle_bond=angle_bond)
    ...
    def create_polymers(self, NP, **kwargs):
        """ 
        create NP polymers
        All the option to create a polymer are defined in the function
        one_polymer in the created module
    ...
    def get_atoms_bonds_angles(self, from_lammps_xyz=None):
        ...
        for molecule in self.molecules:
            ...
            Atom, Bond, Angle = molecule.get_atoms_bonds_angle(start_id=start_id, start_bond=start_bond,
    ...
    def generate_xyz(self, xyz_name, Mass=None, from_lammps_xyz=None):
        ...
        for molecule in self.molecules:
            if coords != []:
                molecule.change_coords(coords[start_id - 1:start_id - 1 + len(molecule.ids)])
            # still assigning a start_id but it should already be correct
            Atom, Bond, Angle = molecule.get_xyz_format_lammps(
                start_id=start_id,
    ...
    def generate_pdb(
            ...
            pdb_name,
            shift=0,
            traduction={"1": "bead", "2": "telo", "3": "ribo", "4": "cent", "5": "spbb", "6": "rcut", "7": "scut"}
    ):
        """
        Generate a pdb file according to the molecules given to the LSimu object
        translation must contain as key the type of bead and as item the pdb name
    ...
    def generate_interactions_o(self, interaction_name, bond="harmonic", SPB=False, radius=10, cutoff=1.15,
                                reducedv_factor=1, khun=1.):
        ...
        for t1 in keyl:
            for t2 in keyl:
                if t2 >= t1:
                    ...
                    if bond == "fene":
                        Bond.append(
                            temp2 % (tybe_b, ene_ratio * ene / (dist * dist), 1.5 * dist, ene_ratio, dist) + "\n")
                    else:
