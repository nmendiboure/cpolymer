...
def create_nucleus(simple=True, oSim=None, dnuc=3, boxf=1.1, Radius=16.6, quad=1):
    ...
    for X in range(fact * Nchromosomes):
        ...
        if X != 11 and X != 11 + Nchromosomes:
            ...
            delta = 0
            if X != 11:
                delta = 1  # to avoid the superposition of the center of the nucleolus

            Sim.add(Polymer(N=len_chrom[X % Nchromosomes],
                            type_bead=[2] + [1] * (d1 - 2) + [3] + [1] * (90 - d1) + [4] * 150 + \
                                      [1] * (len_chrom[X % Nchromosomes] - 150 - 90 - 1) + [2],
                            liaison=liaison,
                            gconstrain=[nucleus],
                            lconstrain=[Point(index=0, position=telo1._v),
                                        Point(index=d1, position=centromere._v),
                                        Point(index=90 + 75, position=(0.66 * Radius, delta, 0)),
    ...
    for i, c in enumerate(dist_centro, 1):
        if c is not None:
            ...
            if not simple:
                Sim.add_extra_bond(mol1=[len(Sim.molecules) - 1, 1], mol2=[i + Nchromosomes, c],
                                   typeb=liaison["3-5"][1])
                # This creates the extra bond between the centromeres.
                # Should be 3-3
    ...
    for k, (bond_size, bond_type) in liaison.items():
        ...
        if bead1 == 5 or bead2 == 5:
            ...
            Sim.add_pair(typep="lj/cut", idpair1=bead1, idpair2=bead2, epsilon=0, sigma=bond_size,
                         cutoff1=bond_size * 1.15)
        else:
            simsoft.add_pair(typep="soft", idpair1=bead1, idpair2=bead2, A=5, rc=bond_size)

            Sim.add_pair(typep="lj/cut", idpair1=bead1, idpair2=bead2, epsilon=1, sigma=bond_size,
                         cutoff1=bond_size * 1.15)

    Sim.add_box(Box([-1 * boxf * Radius, -1 * boxf * Radius, -1 * boxf * Radius],
                    [quad * boxf * Radius, boxf * Radius, boxf * Radius]))
...
Sim, simsoft = create_nucleus(simple=False, dnuc=1)
...
Sim.generate_script(REP + "/nucleus_init.txt",
                    template_name="./tscript", outfile="final.xyz",
